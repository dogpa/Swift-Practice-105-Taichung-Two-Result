//
//  TC2021NoTableViewController.swift
...
class TC2021NoTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "TC2021NOTableViewCell", for: indexPath) as? TC2021NOTableViewCell else{return UITableViewCell()}

        
        cell.villageLabel.text = disAgreeArray[indexPath.row].district + disAgreeArray[indexPath.row].village
        cell.agreeLabel.textColor = .blue
        cell.agreeLabel.text = "同意 \((disAgreeArray[indexPath.row].agree)) 票"
        
        cell.disAgreeLabel.textColor = .red
        cell.disAgreeLabel.text = "不同意 \((disAgreeArray[indexPath.row].disagree)) 票"
        
        cell.percentageLabel.textColor = .green
        cell.percentageLabel.text = "不同意票 \(String(format: "%.3f", ((disAgreeArray[indexPath.row].disAgreePercentage) * 100))) %"
        return cell
    }
    
    override func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        
        return "不同意＞同意投票所：\(disAgreeArray.count) 間"
